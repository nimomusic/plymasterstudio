/**
 * Cloudflare R2 버킷('artist')으로 MP3 파일을 직접 업로드하는 클라이언트 표준 함수
 ...
 * Vercel, Vite, Rollup 어디서든 100% 빌드 에러 없이 R2 버킷에 직접 업로드됩니다.
 */
...
export async function uploadMp3ToR2(formData: FormData): Promise<{ success: boolean; url?: string; error?: string }> {
  ...
  try {
    ...
    // 1. UTC 날짜 및 타임스탬프 계산
    const now = new Date();
    const amzDate = now.toISOString().replace(/[:-]|\.\d{3}/g, ''); // 예: 20260830T115500Z
    const dateStamp = amzDate.substring(0, 8); // 예: 20260830
